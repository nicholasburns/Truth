...
if (not C["Dev"]["Dumptip"]) then return end
...
function MoveIt_Command(id, keystate)									--DEFAULT_CHAT_FRAME:AddMessage(tostring(MoveItFrame:IsVisible()) .. "   " .. tostring(not MoveItFrame:IsVisible()))
	...
	if (id == 1) then
		if (not MoveItFrame:IsShown()) then
			Object = GetMouseFocus()
			Object = MoveIt_GetTopParent(Object)
		end
...
function MoveIt_Move(Move)
	if (Move == true) then
		...
		MoveIt_GetObjPoint()
		Object.Oldx, Object.Oldy = GetCursorPosition()

		MoveItFrame:SetScript("OnUpdate", function()
			local x, y = GetCursorPosition()
			local newX = (x - Object.Oldx) / Object:HiddenGetEffectiveScale()
			local newY = (y - Object.Oldy) / Object:HiddenGetEffectiveScale()

			Object:HiddenSetPoint(Object.HiddenPoint, Object.HiddenRelativeTo or nil, Object.HiddenRelativePoint, Object.HiddenX + newX, Object.HiddenY + newY)
...
function MoveIt_Dettach()
	...
	if ((p[2] == nil) and (Object:GetParent() == UIParent)) then
		p = nil
		MoveIt_Message("[MoveIt] - |cFFFFFF00[" .. tostring(Object:GetName()) .. "]|r is already dettached!", true, true)
		return
	end	p = nil

	local left = Object:GetLeft()
	local bottom = Object:GetBottom()
	local scale = MoveIt_GetTopParent(Object):GetScale()

	Object:SetParent("UIParent")
	Object.parent = Object:GetParent():GetName()
	Object:HiddenClearAllPoints()
	Object:HiddenSetPoint("BOTTOMLEFT", nil, "BOTTOMLEFT", left, bottom)
	Object:HiddenSetScale(scale)
end

function MoveIT_Toggle()
	if (not Object or not Object:GetName()) then
		MoveIt_Message("# MoveIT_Toggle: [" .. "?" .. "] not found.", true, true)
		PlaySoundFile("Sound\\interface\\Error.wav")
		return
	end

	if (Object == WorldFrame or Object == MoveItFrame or Object == UIParent) then
		MoveIt_Message("Invalid f: |cFFFFFF00" .. tostring(Object:GetName()) .. "|r", true, true)
		PlaySoundFile("Sound\\interface\\Error.wav")
		return
	end
	...
	if (MoveItFrame:IsShown()) then
		...
		if (changed and MoveIt_SetOnMouseEvents) then
			MoveIt_SetOnMouseEvents(Object:GetName())
		end

		if (Object.EnableMouse) then
			Object:EnableMouse(Object.IS_MOUSE_ENABLED)			--msg("Object.IS_MOUSE_ENABLED  =   " .. tostring(Object.IS_MOUSE_ENABLED))
		end
		...
		MoveIT_PreventExternalMovement(Object)

		MoveItFrame:SetAllPoints(Object)
		MoveItFrame:SetScale(Object:HiddenGetScale() or 1)
		...
		if (Object.IsMouseEnabled) then
			Object.IS_MOUSE_ENABLED = Object:IsMouseEnabled()		--msg("Object.IS_MOUSE_ENABLED  =   " .. tostring(Object.IS_MOUSE_ENABLED))
		end
	end
...
function MoveIT_RefreshFrame(name)
	...
	if (settings.relativeTo and _G[settings.relativeTo] and settings.point and settings.relativePoint) then
		Object:HiddenSetPoint(settings.point, settings.relativeTo or nil, settings.relativePoint, settings.x, settings.y)
	else
	...
	Object:HiddenSetScale(settings.scale)
	Object:HiddenSetAlpha(settings.alpha)

	if (settings.parent) then
		Object:SetParent(settings.parent)
		Object:HiddenShow()
	end
...
function MoveIt_SaveFrameSettings()
	...
	if (Object.parent) then
		settings.parent = Object.parent
	end

	settings.alpha = MoveIt_math_round(Object:HiddenGetAlpha(), 2)
	settings.scale = MoveIt_math_round(Object:HiddenGetScale(), 2)
	settings.hidden = (not Object:IsShown())

	MoveIt_GetObjPoint()

	settings.point = Object.HiddenPoint
	settings.relativeTo = Object.HiddenRelativeTo
	settings.relativePoint = Object.HiddenRelativePoint
	settings.x = MoveIt_math_round(Object.HiddenX, 2)
	settings.y = MoveIt_math_round(Object.HiddenY, 2)
...
function MoveIt_OnMouseDown(self, button, wheel)
	...
	if (button == LeftButtonString) then
		...
		if (IsAltKeyDown() and IsControlKeyDown() and IsShiftKeyDown()) then
			MoveIt_ResetFrame(Object:GetName(), true)
			MoveIT_Toggle()
			return
		...
		elseif (IsAltKeyDown()) then
			MoveIt_Set_OnUpdateScript(function() Object:HiddenSetAlpha(Object:HiddenGetAlpha() + 0.01) end, wheel)
			changed = true
			return

		-- Scale (+)
		elseif (IsControlKeyDown()) then
			MoveIt_Set_OnUpdateScript(function() Object:HiddenSetScale((Object:HiddenGetScale() or 0) + 0.01) end, wheel)
			changed = true
			return
		...
		-- Move & clamped
			...
			if (ChatFrameEditBox:IsVisible()) then
				ChatFrameEditBox:Insert(Object:GetName())
			end
			Object:HiddenSetClampedToScreen(IsShiftKeyDown())
			MoveIt_Move(true)
			changed = true
			return
		end
	...
	elseif (button == RightButtonString) then
		...
		elseif (IsControlKeyDown()) then
			MoveIt_Set_OnUpdateScript(
				function()			--MoveIt_Message((Object:HiddenGetScale() or 0) - 0.01)
					if ((Object:HiddenGetScale() or 0) - 0.02 > 0) then
						Object:HiddenSetScale((Object:HiddenGetScale() or 0) - 0.01)
					end
				end, wheel
			)
			changed = true
...
function MoveIt_ToolTipUpdate()
	...
	if (Object) then
		...
		if (ObjectParent) then
			sParent = ObjectParent:GetName()							-- Parent
			local ObjectParentParent = Object:GetParent():GetParent()
			if (ObjectParentParent) then
				sParentParent = ObjectParentParent:GetName()				-- ParentParent
			end
		end
		...
		MoveItToolTipFrame:AddDoubleLine("Object:",		Object:GetName(),							1, 1, 1, 0, 1, 0)
		...
		MoveItToolTipFrame:AddDoubleLine("Type:",		Object:GetObjectType() ,						1, 1, 1, 0, 1, 0)						-- DEFAULT_CHAT_FRAME:AddMessage(Object:GetObjectType())

		if (Object:IsObjectType("Frame") or Object:IsObjectType("Button")) then
			MoveItToolTipFrame:AddDoubleLine("Level:",	Object:GetFrameLevel(),						1, 1, 1, 0, 1, 0)
			MoveItToolTipFrame:AddDoubleLine("Strata:",	Object:GetFrameStrata(),						1, 1, 1, 0, 1, 0)
			MoveItToolTipFrame:AddDoubleLine("Scale:",	format("%.2f", Object:HiddenGetScale()),			1, 1, 1, 0, 1, 0)
		end

		MoveItToolTipFrame:AddDoubleLine("Alpha:", 		format("%.2f", Object:HiddenGetAlpha()),			1, 1, 1, 0, 1, 0)
		...
		MoveItToolTipFrame:AddDoubleLine("Point:",		relativePoint, 							1, 1, 1, 0, 1, 0)
		...
		-- Cursor coords
		MoveItToolTipFrame:AddDoubleLine(format("Cursor-X: |cFF00FF00%.0f|r", x), format("Top: |cFF00FF00%.2f|r",	(Object:GetTop() or 0)),	1, 1, 1, 1, 1, 1)
		MoveItToolTipFrame:AddDoubleLine(format("Cursor-Y: |cFF00FF00%.0f|r", y), format("Right: |cFF00FF00%.2f|r", (Object:GetRight() or 0)), 1, 1, 1, 1, 1, 1)
...
function MoveIt_GetObjPoint()
	...
	Object.HiddenPoint = p[1]

	if (p[2]) then
		Object.HiddenRelativeTo = p[2]:GetName()
	else
		Object.HiddenRelativeTo = nil
	end

	Object.HiddenRelativePoint = p[3]
	Object.HiddenX = p[4]
	Object.HiddenY = p[5]
