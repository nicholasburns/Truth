...
if (not lib.MediaTable.background) then			-- BACKGROUND
...
if (not lib.MediaTable.border) then			-- BORDER
...
if (not lib.MediaTable.font) then				-- FONT
	lib.MediaTable.font = {}
end
local SML_MT_font = lib.MediaTable.font
if (locale == "koKR") then
	SML_MT_font["굵은 글꼴"] = [=[Fonts\2002B.TTF]=] SML_MT_font["기본 글꼴"] = [=[Fonts\2002.TTF]=] SML_MT_font["데미지 글꼴"] = [=[Fonts\K_Damage.TTF]=] SML_MT_font["퀘스트 글꼴"] = [=[Fonts\K_Pagetext.TTF]=]
	lib.DefaultMedia["font"] = "기본 글꼴"		LOCALE_MASK = lib.LOCALE_BIT_koKR
-- elseif (locale == "ruRU") then -- ...		NB: Locale removed for clarity
-- elseif (locale == "zhCN") then -- ...		NB: Locale removed for clarity
-- elseif (locale == "zhTW") then -- ...		NB: Locale removed for clarity
else
...
if (not lib.MediaTable.statusbar) then			-- STATUSBAR
...
if (not lib.MediaTable.sound) then				-- SOUND
...
local function rebuildMediaList(mediatype)
	...
	local i = 0
	for k in pairs(mtable) do					-- List can only get larger, so simply overwrite it
		i = i + 1
		mlist[i] = k
	end
	table_sort(mlist)
...
function lib:Register(mediatype, key, data, langmask)
	if (type(mediatype) ~= "string") then error(MAJOR .. ":Register(mediatype, key, data, langmask) - mediatype must be string, got " .. type(mediatype)) end
	if (type(key) ~= "string") then error(MAJOR .. ":Register(mediatype, key, data, langmask) - key must be string, got " .. type(key)) end
