...
function lib.SavePosition(frame)
	local parent = frame:GetParent() or nilParent	-- Works best when frames are parented to nil or UIParent
	local pwidth = parent:GetWidth()
	local pheight parent:GetHeight()

	local s = frame:GetScale()
	local left = frame:GetLeft() * s
	local top = frame:GetTop() * s
	local right = frame:GetRight() * s
	local bottom = frame:GetBottom() * s
...
function lib.RestorePosition(frame)
	...
	if (s) then							-- (frame.lw11origSetScale or frame.SetScale(frame, s)
		frame.SetScale(frame, s)
	else
		s = frame:GetScale()
	end
	...
	if (not point) then						-- we have position, but no point, which probably means we're going from data stored by the addon itself before LibWindow was added to it. It was PROBABLY topleft->bottomleft anchored. Most do it that way.
		...
		queueSavePosition(frame)				-- make it compute a better attachpoint (on next update)
		return
	end
...
function lib.SetScale(frame, scale)
	...
	frame.SetScale(frame, scale)				-- (frame.lw11origSetScale or frame.SetScale)(frame, scale)
