local A, C, G, L = select(2, ...):Unpack()		-- credit: Armory Link (ShestakUI)

if (not C["Chat"]["Enable"] or not C["Chat"]["Armory"]["Enable"]) then
	return
end
...
local realmName = lower(Addon["PlayerRealm"])
