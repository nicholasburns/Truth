...
if (not C["Character"]["ItemLevels"]["Enable"]) then return end
local print = function(...) A.print('Char.ItemLevels', ...) end
...
local function CreateButtonsText(frame)
	for _, slot in pairs(slots) do
		...
		button.text:SetFont(unpack(C["Character"]["ItemLevels"]["Font"]))
		...
		if (slot == "HeadSlot" or slot == "NeckSlot" or slot == "ShoulderSlot" or slot == "BackSlot" or slot == "ChestSlot" or slot == "WristSlot" or slot == "ShirtSlot" or slot == "TabardSlot") then
			button.text:Point("CENTER", button, "CENTER", 42, 0)
		...
		elseif (slot == "MainHandSlot" or slot == "SecondaryHandSlot" or slot == "RangedSlot") then
			button.text:Point("CENTER", button, "CENTER", 0, 42)
		end
...
--	Credit
--------------------------------------------------
--[[ AddOn Name: AsphyxiaUI Version 8.x
	Author: Sinaris, Azilroka
	Description: World of Warcraft User Interface
	Credits:
--]]
