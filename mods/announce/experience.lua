...
if (not C["Announce"]["Experience"]["Enable"]) then
	return
end
...
XP:SetScript('OnEvent', function(self, event, arg1, arg2)
	...
	if (event == 'PLAYER_ENTERING_WORLD') then
		...
		TruthDB = TruthDB or Addon.DefaultDB

		if (not TruthDB["Experience"]) then
			TruthDB["Experience"] = {}
			TruthDB["Experience"]["Enable"] = Addon.DefaultDB["Experience"]["Enable"]
			TruthDB["Experience"]["Counter"] = Addon.DefaultDB["Experience"]["Counter"]
			TruthDB["Experience"]["Timestamp"] = Addon.DefaultDB["Experience"]["Timestamp"]
			TruthDB["Experience"]["Announcements"] = Addon.DefaultDB["Experience"]["Announcements"]
		end
