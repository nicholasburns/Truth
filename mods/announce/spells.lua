...
f:SetScript('OnEvent', function(self, event,  ...)
	...
	if (not sourceName == Addon["PlayerName"]) then return end	-- if (not sourceName == API.myname) then return end
		...
		for i, spells in pairs(spells) do
			...
			--[[ Cast on --> Nil ]]
				...
				--[[ Source cast [Spell] ]]
					...
					SendChatMessage(format('%s used %s', sourceName, spellLink), A.GetAnnounceChannel())															-- SendChatMessage(format(L_ANNOUNCE_SPELL_USE, sourceName, GetSpellLink(spellID)), API.GetAnnounceChannel())
			...
			--[[ Cast on --> Destination ]]
				...
				--[[ Cast by Me ]]
					...
					--[[ [MySpell] cast on -> Destination ]]
						...
						SendChatMessage(format('%s cast on -> %s', spellLink, destName), A.GetAnnounceChannel())														-- SendChatMessage(format(L_ANNOUNCE_SPELL_USE, sourceName, GetSpellLink(spellID) .. ' -> ' .. destName), API.GetAnnounceChannel())
				...
				--[[ NOT Cast by Me ]]
					...
					--[[ Source cast [Spell] -> Destination ]]
						...
						SendChatMessage(format('%ss -> %s', sourceName .. ' used ' .. spellLink, destName), A.GetAnnounceChannel())
		...
		for i, spells in pairs(spells) do
			...
			--[[ Cast on --> Nil ]]
				...
				--[[ [Spell] used ]]
					...
					SendChatMessage(GetSpellLink(spellID), A.GetAnnounceChannel())
			...
			--[[ Cast on --> Destination ]]
				...
				--[[ Player used [MySpell] -> Destination ]]
					...
					SendChatMessage(GetSpellLink(spellID) .. ' -> ' .. destName, A.GetAnnounceChannel())
...
local L_MY_PLAYER_NAME = Addon["PlayerName"]
local SpellList = {
	34477,	-- Misdirection
